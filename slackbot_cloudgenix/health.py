    ...
    if resp.cgx_status:
        ...
        if(len(alarms_list) > 0 ):
            ...
            for alarm_code in alarm_summary_dict.keys():
                alarm_summary_array.append(["Summaries for ALARM: " , pUnderline( str(alarm_code) ) ])
                alarm_summary_array.append(["COUNT: " , pUnderline(str(alarm_summary_dict[alarm_code])) ])
            print_array.append({ style: T1, theader: "ALARM: " + pUnderline(str(alarm['code'])), data: alarm_summary_array })
        else:
...
def health_gcloud_information(sdk, idname, global_vars):
    ...
    print_array.append({ style: P1, data: "GOOGLE Cloud STATUS via API" })
...
def cp_get_router(router_serial, request_headers, base_url='https://www.cradlepointecm.com/api', api="/v2/routers/"):
    url = base_url + api 
    ...
    while True:
        ...
        req = requests.get(url, headers=request_headers)
        routers_resp = req.json()
        for cp_device in routers_resp.get("data"):
            if cp_device.get("serial_number") == router_serial:
                #print("Found Device :", cp_device.get("name"))
                #print("        STATE:", cp_device.get("state"))
                #print("     Hardware:", cp_device.get("full_product_name"))
                #print("          URL:", cp_device.get("resource_url"))
                #print("       SERIAL:", cp_device.get("serial_number"))
                #print("           ID:", cp_device.get("id"))
                return cp_device
        if not(routers_resp['meta']['next']):
            break
    return None

def cp_get_active_router_interface_obj(router_id, request_headers, base_url='https://www.cradlepointecm.com/api', api="/v2/net_devices/"):
    url = base_url + api + '?router__in=' + str(router_id)
    #url = base_url + api + '?id=' + str(router_id)
    req = requests.get(url, headers=request_headers)
    netdevices_resp = req.json()
    
    for net_interfaces in netdevices_resp.get("data"):
        if net_interfaces.get("summary") == "connected":
            #print("Found Network Interface for Router")
            #print("           id",net_interfaces['id'])
            #print("     hostname",net_interfaces['hostname'])
            #print("         imei",net_interfaces['imei'])
            #print("      carrier",net_interfaces['carrier'])
            return net_interfaces
    return None

def cp_get_net_interface_metrics(netid, request_headers, date_start, date_end, base_url='https://www.cradlepointecm.com/api', api="/v2/net_device_signal_samples/"):
    url = base_url + api + '?net_device=' + str(netid) + "&created_at__gt=" + str(date_start) + "&created_at__lt=" + str(date_end) + "&limit=9000"
    req = requests.get(url, headers=request_headers)    
    response = req.json()
...
def cp_metric_classifier(value, expected, warn_percentage_as_decimal, error_percentage_as_decimal):
    warn_threshold = expected * warn_percentage_as_decimal
    fail_threshold = expected * error_percentage_as_decimal
    if (value < fail_threshold ):
        return pFail(str(value))
    if (value > 0) and (value >= expected + (expected * error_percentage_as_decimal) ):
        return pExceptional(str(value))
    if (value <  warn_threshold ):
        return pWarn(str(value))
    return pPass(str(value))

def cradlepoint_validate_auth(cradlepoint_request_headers):
    if (type(cradlepoint_request_headers) == dict) and                       \
            (len(cradlepoint_request_headers['X-ECM-API-ID']) > 5) and      \
            (len(cradlepoint_request_headers['X-ECM-API-KEY']) > 5) and     \
            (len(cradlepoint_request_headers['X-CP-API-ID']) > 5) and       \
            (len(cradlepoint_request_headers['X-CP-API-KEY']) > 5):
        return True
    return False


def health_cradle_point_cloudblade(sdk, idname, global_vars):
    if ('cradlepoint_request_headers' not in global_vars.keys()):
        return None ###No Auth Found
    if not cradlepoint_validate_auth(global_vars['cradlepoint_request_headers']):
        return None
    
    request_headers = global_vars['cradlepoint_request_headers']

    dt_now       = datetime.now()
    dt_24hrsago  = datetime.today() - timedelta(hours=24)
    date_end = str(dt_now.strftime("%Y-%m-%dT%H:%M:%SZ"))
    date_start = str(dt_24hrsago.strftime("%Y-%m-%dT%H:%M:%SZ"))

    ###Perform Metrics loop setup
    metrics_list = {    ### EXPECTED, WARN PERCENTAGE, ERROR PERCENTAGE
                    'rsrp' : [ -84,     1.226,  1.3214 ],
                    'rsrq' : [ -5,      1.8,    2.4],
                    'sinr' : [ 12.5,    .8,     .55],
                    'signal_percent' : [ 100,   .8,     .6],
                }
    metrics_dict = {}
    ...
    if resp.cgx_status:
        ...
        if (len(element_list) >= 0):
            ...
            for element in element_list:                            #Loop through each EVENT in the EVENT_LIST    
                if (element['site_id'] == site_id):
                    interfaces_resp = sdk.get.interfaces(site_id, element['id'])
                    print_array.clear()
                    print_array.append({ style: P1, data: "CradlePoint DATA for device on ION " + str(pBold(element['name'])) })
                    for interface in interfaces_resp.cgx_content.get("items"):
                        description = interface['description']
                        if description:
                            cradlepoint_serial_re = re.search("SERIAL:([a-zA-Z0-9]*)", description, re.IGNORECASE)
                            if cradlepoint_serial_re:
                                cradlepoint_serial = cradlepoint_serial_re.group(1)
                                ###FIND ROUTER Device based on Serial
                                router_object = cp_get_router(cradlepoint_serial, request_headers)
                                if (router_object):
                                    ###FIND the active LTE interface on the ROUTER
                                    cp_active_interface = cp_get_active_router_interface_obj(router_object['id'], request_headers)
                                    if (cp_active_interface):
                                        active_interface_id = cp_active_interface['id']
                                        ###GET Interface Metrics
                                        interface_metrics.clear()
                                        interface_metrics = cp_get_net_interface_metrics(active_interface_id, request_headers, date_start, date_end)
                                        if (interface_metrics):             
                                            print_array.append({ style: H1, data: "Measurements for CradlePoint Device: '" + str(pBold(router_object['name'])) + "'  On LTE Interface " +  pBold(str(cp_active_interface['name'])) + " Connected to ION port " + pBold(str(interface['name']))})
                                            metrics_dict.clear()
                                            for item in metrics_list.keys():
                                                metrics_dict[item] = []
                                            ###Loop through metrics for each datapoint and for each metric to be collected
                                            for metric in interface_metrics:
                                                metric_time = metric['created_at']
                                                if "first_metric" not in metrics_dict.keys():
                                                    metrics_dict["first_metric"] = metric
                                                if metric_time > metrics_dict["first_metric"]['created_at']:
                                                    metrics_dict["first_metric"] = metric
                                                for item in metrics_list.keys():
                                                    if metric[item] == None:
                                                        metrics_dict[item].append(0)
                                                    else:
                                                        metrics_dict[item].append(metric[item])
                                            for item in metrics_list.keys():
                                                np_array = np.array(metrics_dict[item])
                                                print_array.append({ style: T1, theader: str(item) + " - Measurements in the past 24 Hours (" + pPass("GOOD:" + str(metrics_list[item][0])) + pWarn(" FAIR:" + str(round(metrics_list[item][1]*metrics_list[item][0],2))) + pFail(" POOR:" + str(round(metrics_list[item][2]*metrics_list[item][0],2))) +   ")", data:[ 
                                            #        [ "25th percentile",        cp_metric_classifier(round(np.percentile(np_array,25),3)    ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                                    ["Current Reading",     cp_metric_classifier(metrics_dict["first_metric"][item]          ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                                    ["50th Percentile(AVG)",    cp_metric_classifier(round(np.average(np_array),3)          ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                            #        ["75th percentile"     ,    cp_metric_classifier(round(np.percentile(np_array,75),3)    ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                                    ["95th percentile"      ,   cp_metric_classifier(round(np.percentile(np_array,95),3)    ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                            #        ["Min Value"        ,       cp_metric_classifier(round(np.amin(np_array),3)             ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                                    ["Max Value"        ,       cp_metric_classifier(round(np.amax(np_array),3)             ,metrics_list[item][0],metrics_list[item][1],metrics_list[item][2])     ],
                                                                            ] })
                                if len(print_array) > 1: uprint(print_array)


def site_health_check(raw_api, site_id, sdk, idname, site_name):
    ...
    ###Fill out the below to include CradlePoint Health Information.
    ### This looks at the ION/Element interface description for a Cradlepoint Serial in the format of SERIAL:YYXXXXXXXXXXXX
    ### and uses the Cradlepoint Netcloud API to pull health information
    cradlepoint_request_headers = {}
    cradlepoint_request_headers['X-ECM-API-ID']     = '...'
    cradlepoint_request_headers['X-ECM-API-KEY']    = '...'
    cradlepoint_request_headers['X-CP-API-ID']      = '...'
    cradlepoint_request_headers['X-CP-API-KEY']     = '...'
    cradlepoint_request_headers['Content-Type']     = 'application/json'
    global_vars['cradlepoint_request_headers'] = cradlepoint_request_headers
    ...
    health_cradle_point_cloudblade(sdk, idname, global_vars)
