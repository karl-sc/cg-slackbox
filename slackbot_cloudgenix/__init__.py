...
sys.path.append('./slackbot_cloudgenix/')
...
check_load_last_modified = {}
check_load_last_modified['health'] = os.path.getmtime(os.path.abspath(health.__file__))
check_load_last_modified['dashboard'] = os.path.getmtime(os.path.abspath(health.__file__))
...
def help(message):
    ...
    message_text = 'I can take the following commands (for now):\n' \
                   ...
                   '"Show application <app name> at <site name>"\n' \
...
def show_site_health(message, site_string):
    ...
    if sdk.tenant_id:
        ...
        if percent > 50:
            ...
            last_modified_time = os.path.getmtime(os.path.abspath(health.__file__))
            
            print("Health last loaded time:",check_load_last_modified['health'])
            print("Health last modified time:",last_modified_time)
            if (last_modified_time > check_load_last_modified['health']):
                importlib.reload(health)
                print("reloading site health module")
                tmp_msg_buffer = '[ {"type": "divider"},{	"type": "section","text": {"type": "mrkdwn","text": "*Coding changes detected... Reloading health check module...*"} } ]'
                constructed_raw_message = {
                "channel": raw_api.channel_id,
                "as_user": raw_api.self_id,
                "blocks": tmp_msg_buffer
                    }
                raw_api.Slacker.chat.post('chat.postMessage', data=constructed_raw_message)
            
            site_name = "{0}".format(choice)
            health.site_health_check(raw_api, sites_n2id[choice], sdk, idname, site_name )
            ###########
        # not even close..
        else:
...
@respond_to('dashboard', re.IGNORECASE)
def show_dashboard(message):
    ...
    if sdk.tenant_id:
        ...
        last_modified_time = os.path.getmtime(os.path.abspath(dashboard.__file__))        
        print("dashboard - last loaded time:",check_load_last_modified['dashboard'])
        print("dashboard - last modified time:",last_modified_time)
        if (last_modified_time > check_load_last_modified['dashboard']):
            importlib.reload(dashboard)
            print("reloading site dashboard module")
            tmp_msg_buffer = '[ {"type": "divider"},{	"type": "section","text": {"type": "mrkdwn","text": "*Coding changes detected... Reloading dashboard check module...*"} } ]'
            constructed_raw_message = {
            "channel": raw_api.channel_id,
            "as_user": raw_api.self_id,
            "blocks": tmp_msg_buffer
        ...
        dashboard.dashboard_health_check(raw_api, sdk, idname )
    else:
        message.react(BAD_RESPONSE)
        message.reply(CGX_API_ERROR_MSG)
